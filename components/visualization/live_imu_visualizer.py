...
class WebSocketHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/ws':
            ...
            self.end_headers()
            with open('live_imu_3d.html', 'r') as f:
    ...
    def handle_websocket(self):
        ...
        self.send_header('Sec-WebSocket-Accept', 'dummy')
        self.end_headers()
        ...
        try:
            while True:
                ...
                with data_lock:
                    if latest_data:
                        # Simple WebSocket frame (not fully compliant but works for demo)
                        data_bytes = latest_data.encode('utf-8')
                        frame = b'\x81' + bytes([len(data_bytes)]) + data_bytes
...
def start_web_server():
    ...
    server = HTTPServer(('localhost', 8002), WebSocketHandler)
    print("Web server started on http://localhost:8002")
    server.serve_forever()

if __name__ == "__main__":
    print("ESP32 Live IMU 3D Visualizer")
    print("============================")
