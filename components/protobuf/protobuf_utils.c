...
// Estimated size per sample in protobuf format (with margin for 2-byte tags on
// fields 16+) Actual size ~125 bytes: fields 1-15 (~74B) + fields 16-25 (~48B)
// + submessage wrapper (~3B)
#define ESTIMATED_SAMPLE_SIZE 140

// Helper to copy bno055_sample_t to goldenform_ImuSample
static void sample_to_proto(const bno055_sample_t *sample,
                            goldenform_ImuSample *msg) {
...
esp_err_t protobuf_encode_sample(const bno055_sample_t *sample, uint8_t *buffer,
  ...
  goldenform_ImuSample msg = goldenform_ImuSample_init_zero;
  ...
  // Encode
  if (!pb_encode(&stream, goldenform_ImuSample_fields, &msg)) {
...
esp_err_t protobuf_encode_batch(const bno055_sample_t *samples,
                                size_t sample_count, uint8_t *buffer,
                                size_t buffer_size, size_t *encoded_size) {
  ...
  if (sample_count > MAX_BATCH_SAMPLES) {
    ESP_LOGW(TAG, "Batch size %zu exceeds max %d, truncating", sample_count,
             MAX_BATCH_SAMPLES);
    sample_count = MAX_BATCH_SAMPLES;
  }

  // Create batch message with fixed array
  // Use heap_caps_calloc from internal RAM only (required for proper free()
  // behavior)
  goldenform_ImuSampleBatch *batch =
      heap_caps_calloc(1, sizeof(goldenform_ImuSampleBatch),
  ...
  // Encode
  bool encode_ok = pb_encode(&stream, goldenform_ImuSampleBatch_fields, batch);
...
esp_err_t protobuf_decode_sample(const uint8_t *buffer, size_t buffer_size,
  ...
  if (!pb_decode(&stream, goldenform_ImuSample_fields, &msg)) {
...
esp_err_t protobuf_decode_batch(const uint8_t *buffer, size_t buffer_size,
  ...
  if (!buffer || !samples || !decoded_count || buffer_size == 0 ||
      max_samples == 0) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  goldenform_ImuSampleBatch *batch =
      calloc(1, sizeof(goldenform_ImuSampleBatch));
  if (!batch) {
  ...
  if (!pb_decode(&stream, goldenform_ImuSampleBatch_fields, batch)) {
  ...
  size_t to_copy =
      (batch->samples_count < max_samples) ? batch->samples_count : max_samples;
...
esp_err_t protobuf_write_delimited(FILE *file, const bno055_sample_t *samples,
  ...
  // Allocate encoding buffer from internal RAM only (required for proper free()
  // behavior)
  ...
  if (!buffer) {
    ESP_LOGE(TAG,
             "Failed to allocate %zu bytes from internal RAM for protobuf "
             "encode buffer",
             buffer_size);
    return ESP_ERR_NO_MEM;
  }
  ...
  esp_err_t err = protobuf_encode_batch(samples, sample_count, buffer,
...
esp_err_t protobuf_read_delimited(FILE *file, bno055_sample_t *samples,
  ...
  *decoded_count = 0;
  ...
  if (!buffer) {
    ESP_LOGE(TAG, "Failed to allocate %lu bytes for protobuf decode",
             (unsigned long)len);
    return ESP_ERR_NO_MEM;
  }
  ...
  // Decode the batch
  esp_err_t err =
      protobuf_decode_batch(buffer, len, samples, max_samples, decoded_count);
 ...
 *
 * @param file File pointer (must be positioned at start of 4-byte length
 * prefix)
...
esp_err_t protobuf_peek_batch_size(FILE *file, size_t *sample_count) {
  ...
  bool decode_ok = pb_decode(&stream, goldenform_ImuSampleBatch_fields, batch);
...
esp_err_t protobuf_skip_delimited(FILE *file) {
  ...
  if (fseek(file, (long)len, SEEK_CUR) != 0) {
    ESP_LOGE(TAG, "Failed to seek past protobuf message of %lu bytes",
             (unsigned long)len);
    return ESP_FAIL;
  }
